...
export const beckettParser = {
  provider: "Beckett",
  matches(sourceUrl: string, sourceName?: string) {
    return /beckett\.com/i.test(sourceUrl) || /beckett/i.test(sourceName || "");
  },
  parse(html: string, context: ChecklistParserContext): ParsedChecklistResult {
    ...
    return normalizeCandidateRows("Beckett", context, candidates);
  }
};
...
function parsePlainChecklistText(html: string): ChecklistCandidateRow[] {
  ...
  let sectionTeam: string | null = null;
  ...
  for (const line of lines) {
    const headingTeam = teamFromChecklistHeading(line);
    if (headingTeam) {
      sectionTeam = headingTeam;
      subset = null;
      continue;
    }
    ...
    if (candidate) {
      candidates.push({
        ...candidate,
        team: candidate.team || sectionTeam
      });
    }
  }
...
function isLikelySubsetHeading(line: string) {
  ...
  if (/\b(checklist top|view the|buy on ebay|shop|comment|reply|privacy|terms)\b/i.test(text)) return false;
  return /(?:base|prospects?|autographs?|inserts?|rookies?|favorites?|spotlights?|chrome|bowman|scouts|ultimate|modern|sights|invicta|contenders?|prizm|optic|mosaic|phoenix|select|certified|illusions|elite|zenith|chronicles|legacy|score|absolute|finest|heritage|update|series)\b/i.test(text);
}
